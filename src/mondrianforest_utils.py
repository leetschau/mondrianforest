    ...
    def evaluate_predictions(self, data, x, y, settings, param, weights, print_results=True):
        ...
        if print_results:
            if settings.optype == 'class':
                ...
                print('Averaging over all trees, mse = %f, rmse = %f, log_prob = %f' %
                        (metrics['mse'], math.sqrt(metrics['mse']), metrics['log_prob']))
...
def compute_left_right_statistics(data, param, cache, train_ids, feat_id_chosen, \
    ...
    if settings.verbose >= 2:
        ...
        print('y (left) = %s\ny (right) = %s' %
                (data['y_train'][train_ids_left], data['y_train'][train_ids_right]))
